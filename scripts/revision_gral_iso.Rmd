...
camara <- collect(tbl(base_input, "Camara")) %>%
  ...
  inner_join(conglomerado, by = c("conglomerado_muestra_id" = "id"))
  
archivo_camara <- collect(tbl(base_input, "Archivo_camara"))

archivos <- archivo_camara %>%
  right_join(select(camara, cgl, id, fecha_inicio, hora_inicio,
...
_mun_: municipio, _# sitios_: número de sitios existentes.  



```{r, echo=FALSE, warning=FALSE, message=FALSE}
if(flag_cgl){
  colnames(tab_cgl) <- c("cgl", "fecha", "edo.", "mun.", "# sitios")
  kable(tab_cgl %>% arrange(cgl) %>% convert_to_encoding())
}else{
...
if(flag_grabadora){
  colnames(tab_grabadora) <- c("cgl", "inicio", "término", "# archivos", 
    "% audio", "% ultra.")
  kable(tab_grabadora %>% arrange(cgl) %>% distinct() %>% convert_to_encoding())
}else{
  kable("No hay registros")
}
```
...
if(flag_er_extra){
  colnames(tab_er_extra) <- c("cgl", "fecha", "# obs.",  "% especimen", 
    "% resto", "# archivos")
  kable(tab_er_extra %>% arrange(cgl) %>% distinct() %>% convert_to_encoding())  
}else{
  kable("No hay registros")
}
```
