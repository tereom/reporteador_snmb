...
base_input <- src_sqlite(base_archivos)
...
Imagen_sitio_ruta <- Imagen_sitio_info %>%
  ...
  mutate(
    ...
    salida = paste(ruta_archivos, "/", nombre_anio_mes, "/referencias/", id, "__",
...
Imagen_camara_ruta <- Imagen_camara_info %>%
  ...
  mutate(
    ...
    salida = paste(ruta_archivos, "/", nombre_anio_mes, "/referencias/", nuevo_nombre,
      sep = "")
  ) %>%
  select(
    nombre_anio_mes,
...
Archivo_camara_ruta <- Archivo_camara_info %>%
  ...
  mutate(
    nuevo_nombre = gsub("(.*\\.).*\\.(.*\\.).*\\.", "\\1\\2", archivo),
    salida = paste(
      ruta_archivos, "/", nombre_anio_mes, "/fotos_videos/", indicador, "_",
      nuevo_nombre, sep = ""
      )
  ) %>%
  select(
...
Archivo_grabadora_ruta <- Archivo_grabadora_info %>%
  ...
  mutate(
    salida = paste(ruta_archivos, "/", nombre_anio_mes, "/", carpeta, "/", nuevo_nombre,
      sep = "")
  ) %>%
  select(
    nombre_anio_mes,
...
Archivo_especie_invasora_ruta <- Archivo_especie_invasora_info %>%
  ...
  mutate(
    nuevo_nombre = gsub("(.*\\.).*\\.(.*\\.).*\\.", "\\1\\2", archivo),
    salida = paste(ruta_archivos, "/", nombre_anio_mes, "/especies_invasoras/",
      nuevo_nombre, sep = "")
  ) %>%
  select(
    nombre_anio_mes,
...
Archivo_huella_excreta_ruta <- Archivo_huella_excreta_info %>%
  ...
  mutate(
    nuevo_nombre = gsub("(.*\\.).*\\.(.*\\.).*\\.", "\\1\\2", archivo),
    salida = paste(ruta_archivos, "/", nombre_anio_mes, "/huellas_excretas/",
      nuevo_nombre, sep = "")
  ) %>%
  select(
    nombre_anio_mes,
...
Archivo_especie_invasora_extra_ruta <- Archivo_especie_invasora_extra_info %>%
  ...
  mutate(
    nuevo_nombre = gsub("(.*\\.).*\\.(.*\\.).*\\.", "\\1\\2", archivo),
    salida = paste(ruta_archivos, "/", nombre_anio_mes, "/registros_extra/",
      nuevo_nombre, sep = "")
  ) %>%
  select(
    nombre_anio_mes,
...
Archivo_conteo_ave_ruta <- Archivo_conteo_ave_info %>%
  ...
  mutate(
    nuevo_nombre = gsub("(.*\\.).*\\.(.*\\.).*\\.", "\\1\\2", archivo),
    salida = paste(ruta_archivos, "/", nombre_anio_mes, "/otros/", nuevo_nombre, sep = "")
  ) %>%
  select(
    nombre_anio_mes,
    entrada = archivo,
...
dir.create(ruta_archivos)

ruta_archivos_nombre_anio_mes <- paste(ruta_archivos, "/", Conglomerado_carpetas$nombre_anio_mes,
  sep = "")
lapply(ruta_archivos_nombre_anio_mes, dir.create, recursive = TRUE)
...
producto_cartesiano <- expand.grid(x = ruta_archivos_nombre_anio_mes, y = subcarpetas)
ruta_archivos_nombre_anio_subcarpetas <- paste(producto_cartesiano$x, producto_cartesiano$y, sep = "/")
lapply(ruta_archivos_nombre_anio_subcarpetas, dir.create)
...
#   mutate(
#     ruta_destino = paste(ruta_archivos, "/", nombre_anio_mes, "/", nombre_nuevo, sep = ""),
...
#momento (no para todos los archivos en la carpeta "ruta_archivos")
# Conglomerados_formulario <- ldply(Conglomerado_carpetas$nombre_anio_mes,
#   function(x){
#     ruta_carpeta <- paste(ruta_archivos, x, sep = "/")
...
if(nrow(Archivos_no_encontrados) > 0)
  write.csv(Archivos_no_encontrados, file = paste0(
    ruta_entrega, "/", entrega, "_no_encontrados.csv"), row.names = FALSE)
...
if(nrow(Rutas_origen_destino_fail) > 0)
  write.csv(Rutas_origen_destino_fail, file = paste0(
    dir_padre, "/", entrega, "_fallidos_otros.csv"), row.names = FALSE)

# Reporte de número de formatos por conglomerado
#write.csv(Conglomerados_formulario, file = paste0(
  #dir_padre, "/", entrega, "_numero_formatos.csv"), row.names = FALSE)
...
ruta_archivos_no_registrados <- paste(ruta_archivos, "/", fecha_actual, "_", "no_reg",
  sep = "")
dir.create(ruta_archivos_no_registrados)

ruta_archivos_no_registrados_subcarpetas <- paste(ruta_archivos_no_registrados, c(
  "fotos_videos",
  "audio",
  "archivos_pdf"), sep = "/")

lapply(ruta_archivos_no_registrados_subcarpetas, dir.create)
...
if(nrow(Archivos_no_registrados_origen_destino) > 0)
